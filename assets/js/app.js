...
// ═══════════════════════════════════════════════════════════════════════════════
// V37.3 — CORREÇÃO DO RENDER EM BRANCO
// O loading(false) legado apaga #content. Os renders V37 agora substituem
// diretamente o spinner pelo HTML final e não executam a limpeza posterior.
// ═══════════════════════════════════════════════════════════════════════════════
const VERSAO_TESOURARIA_V373 = '37.3';

const setViewBaseV373 = setView;
setView = function(v){
  setViewBaseV373(v);
  if(v==='caixa'){
    const top=document.getElementById('topbar-right');
    if(top) top.innerHTML='<span style="font-size:11px;color:var(--texto-muted);font-weight:700;letter-spacing:.6px">TESOURARIA · V37.3</span>';
  }
};
window.setView = setView;

window.diagnosticoTesourariaV373 = function(){
  const base = typeof diagnosticoTesourariaV372 === 'function'
